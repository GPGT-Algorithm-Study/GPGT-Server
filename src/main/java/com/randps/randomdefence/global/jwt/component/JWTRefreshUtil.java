...
public class JWTRefreshUtil {
  ...
  public String getHeaderToken(HttpServletRequest request, String type) {
    return type.equals("Access") ? request.getHeader(ACCESS_TOKEN)
  ...
  public TokenDto createAllToken(String nickname) {
    return new TokenDto(
        createToken(nickname, "Access", principalDetailsService.loadRolesByUsername(nickname)),
        createToken(nickname, "Refresh", principalDetailsService.loadRolesByUsername(nickname)));
  }


  public String createToken(String nickname, String type, String roles) {

    Date date = new Date();

    long time = type.equals("Access") ? ACCESS_TIME : REFRESH_TIME;

    return JWT.create().withSubject(nickname)
        .withExpiresAt(new Date(System.currentTimeMillis() + time)).withIssuedAt(date)
        .withClaim("bojHandle", nickname).withClaim("roles", roles).sign(this.getSign());
  }
...
//            JWT.parserBuilder().setSigningKey(key).build().parseClaimsJws(token);
   ...
   * 토큰에서 인증 권한을 반환한다.
   */
  public String getRoles(String token) throws CertificateExpiredException {
    try {
      return JWT.require(this.getSign()).build().verify(token).getClaim("roles").asString();
    } catch (Exception ex) {
      throw new CertificateExpiredException("토큰 인증이 실패했습니다.");
    }
  }
  ...
  public Boolean refreshTokenValidation(String token) {
    ...
    // DB에 저장한 토큰 비교
    Optional<RefreshToken> refreshToken = refreshTokenRepository.findByBojHandle(
        getBojHandleFromToken(token));
